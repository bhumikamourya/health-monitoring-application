...
function HeartRateChart({ days = 7, target = 75 }) {
  ...
  const getHRColor = (value) => {
    if (value < 60) return "#f87171";    // 🔴 Low
    if (value <= 100) return "#dc2626";  // 🟠 Normal
    return "#991b1b";                    // 🔴 High
  };
  ...
  return (
    ...
    >
      <h2 style={{ marginBottom: 8, color: "#b91c1c" }}>
        <i className="fa-solid fa-heart" style={{ color: "#dc2626" }}></i> Heart Rate
      </h2>

      <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
        <div><b>Today:</b> {todayTotal} bpm</div>
        <div><b>Avg ({days}d):</b> {avg} bpm</div>
        <div><b>Target:</b> {target} bpm</div>
      </div>
      ...
      >
        <div><i style={{ color: "#f87171" }} className="fa-solid fa-heart"></i> Low</div>
        <div><i style={{ color: "#dc2626" }} className="fa-solid fa-heart"></i> Normal</div>
        <div><i style={{ color: "#991b1b" }} className="fa-solid fa-heart"></i> High</div>
      </div>
    </div>
  );
}
...
function CholesterolChart({ days = 7, ldlTarget = 100, hdlTarget = 40 }) {
  ...
  return (
    ...
    >
      <h2 style={{ marginBottom: 8, color: "#0c4a6e" }}>
        <i className="fa-solid fa-dna" style={{ color: "#0284c7" }}></i> Cholesterol
      </h2>

      <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
        <div><b>Today:</b> {todayTotal} mg/dL</div>
        <div><b>Avg ({days}d):</b> {avg} mg/dL</div>
        <div><b>Targets:</b> LDL ≤ {ldlTarget}, HDL ≥ {hdlTarget}</div>
      </div>
      ...
      >
        <div><i style={{ color: "#f59e0b" }} className="fa-solid fa-dna"></i> LDL</div>
        <div><i style={{ color: "#10b981" }} className="fa-solid fa-dna"></i> HDL</div>
        <div><i style={{ color: "#0c4a6e" }} className="fa-solid fa-line"></i> Target Lines</div>
      </div>
    </div>
  );
}
...
function OxygenChart({ days = 7, target = 95 }) {
  ...
  const getOxygenColor = (value, target) => {
    if (value < 90) return "#f65a45ff";    // 🔴 Low
    if (value < 95) return "#6a89dfff";    // 🟡 Medium
    return "#1d4ed8";                      // 🔵 Good
  };
  ...
  return (
    ...
    >
      <h2 style={{ marginBottom: 8, color: "#1e40af" }}>
        <i className="fa-solid fa-lungs" style={{ color: "#1d4ed8" }}></i> Blood Oxygen
      </h2>
      ...
      >
        <div><i style={{ color: "#f65a45ff" }} className="fa-solid fa-lungs"></i> Low</div>
        <div><i style={{ color: "#6a89dfff" }} className="fa-solid fa-lungs"></i> Medium</div>
        <div><i style={{ color: "#1d4ed8" }} className="fa-solid fa-lungs"></i> Good</div>
      </div>
    </div>
  );
}
